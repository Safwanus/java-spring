...
<html>
   ...
   <body>
      ...
      <form method="POST" action="/openForm">
         <button name="idList" class="btn btn-info" type="submit" value="${idList}" ><strong>Ajouter un nouvel element dans <span style="color: #FFCD39">${title}</span></strong></button>
      </form>
      <div class="table-responsive-sm">
         <table class="table">
            ...
            <tr>
               ...
               <td>${system.getDcreation()}</td>
               <td>${system.getDmodification()}</td>
               <td>
               ...
               <td>
                  <form method="POST" action="/shareFormElem">
                     <button name="id" class="btn btn-success" type="submit" value="${system.getId()}" >Partager </button>
                  </form>
               </td>
               <td>
                  <form method="POST" action="/tagForm">
                     <button name="id" class="btn btn-dark" type="submit" value="${system.getId()}" >Tag </button>
                  </form>
               </td>
               <td>
                  <form method="POST" action="/status">
                     ...
                     <select name="stat" id="stat">
                        <option value="<#list status as stat><#if stat.getEId() == system.getId()>${stat.getOption()}</#if></#list>"><#list status as stat><#if stat.getEId() == system.getId()>${stat.getOption()}</#if></#list></option>
                        <option value="A faire">A faire</option>
                        <option value="Fait">Fait</option>
                        <option value="Supprimer">Supprimer</option>
                     </select>
                     </br>
                     <button name="id" class="btn btn-secondary" type="submit" value="${system.getId()}" >Valider </button>
                  </form>
               </td>
            </tr>
            </#list>
      ...
      <form method="POST" action="/openFormAddSubList">
         <button name="idList2" class="btn btn-info" type="submit" value="${idList}" ><strong>Ajouter une sous liste dans <span style="color: #FFCD39">${title}</span></strong></button>
      </form>
      <div class="table-responsive-sm">
      <table class="table">
         <tr>
